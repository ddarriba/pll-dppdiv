...
/* start Pthreads with a single tree */
void startPthreads(tree *tr) {
	startPthreadsGeneric(&tr, 1);
}

/* start Pthreads with multiple trees */
void startPthreadsGeneric(tree **tr, int numberOfTrees)
{
	pthread_t *threads;
	pthread_attr_t attr;
	int rc, t, i;
	...
	timeBuffer = calloc(NUM_PAR_JOBS * tr[0]->numberOfThreads, sizeof(double));
#endif

	pthread_attr_init(&attr);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);

	threads = (pthread_t *)malloc((size_t)tr[0]->numberOfThreads * sizeof(pthread_t));
	tData = (threadData *)malloc((size_t)tr[0]->numberOfThreads * sizeof(threadData));

	barrierBuffer = (volatile char *) malloc(sizeof(volatile char) * (size_t)tr[0]->numberOfThreads);

	for(t = 0; t < tr[0]->numberOfThreads; t++)
	barrierBuffer[t] = 0;

	for(t = 1; t < tr[0]->numberOfThreads; t++)
	{
		tData[t].tr = (tree**)malloc(numberOfTrees * sizeof(tree*));
		tData[t].numberOfTrees = numberOfTrees;
...
void perSiteLogLikelihoodsPthreads(tree *tr, double *lhs, int n, int tid) {
	...
	for (model = 0; model < tr->NumberOfModels; model++) {
		...
		boolean execute = ((tr->manyPartitions
				&& isThisMyPartition(tr, tid, model)) || (!tr->manyPartitions));
		...
		if (execute)
			for (i = tr->partitionData[model].lower;
					i < tr->partitionData[model].upper; i++) {
				...
				if (tr->manyPartitions || (i % n == tid)) {
					...
					case CAT:
						l =
								evaluatePartialGeneric(tr, localIndex,
										tr->partitionData[model].perSiteRates[tr->partitionData[model].rateCategory[localIndex]],
										model);
						break;
					case GAMMA:
...
void computeFractionMany(tree *localTree, int tid) {
	...
	for (model = 0; model < localTree->NumberOfModels; model++) {
		if (isThisMyPartition(localTree, tid, model)) {
			localTree->partitionData[model].width =
					localTree->partitionData[model].upper
							- localTree->partitionData[model].lower;
...
void computeFraction(tree *localTree, int tid, int n) {
	...
	for (model = 0; model < localTree->NumberOfModels; model++) {
		int width = 0;
		for (i = localTree->partitionData[model].lower;
				i < localTree->partitionData[model].upper; i++)
			if (i % n == tid)
				width++;
...
static int partCompare(const void *p1, const void *p2) {
	partitionType *rc1 = (partitionType *) p1, *rc2 = (partitionType *) p2;

	int i = rc1->partitionLength, j = rc2->partitionLength;
...
void multiprocessorScheduling(tree *tr, int tid) {
	int s, model, modelStates[2] = { 4, 20 }, numberOfPartitions[2] = { 0, 0 },
	...
	tr->partitionAssignment = (int *) malloc(
			(size_t) tr->NumberOfModels * sizeof(int));
	...
	for (model = 0; model < tr->NumberOfModels; model++) {
		...
		for (s = 0; s < arrayLength; s++) {
			exists = exists
					|| (tr->partitionData[model].states == modelStates[s]);
	...
	for (s = 0; s < arrayLength; s++) {
		if (numberOfPartitions[s] > 0) {
			...
			int i, k,
					...
					n = processes,
#endif
					p = numberOfPartitions[s], *assignments = (int *) calloc(
							(size_t) n, sizeof(int));

			partitionType *pt = (partitionType *) malloc(
					sizeof(partitionType) * (size_t) p);

			for (i = 0, k = 0; i < tr->NumberOfModels; i++) {
				if (tr->partitionData[i].states == modelStates[s]) {
					pt[k].partitionNumber = i;
					pt[k].partitionLength = tr->partitionData[i].upper
							- tr->partitionData[i].lower;
			...
			for (i = 0; i < p; i++) {
				...
				assert(
						pt[i].partitionNumber >= 0 && pt[i].partitionNumber < tr->NumberOfModels);
			...
			if (tid == 0) {
				for (i = 0; i < n; i++)
					printBothOpen(
							"Process %d has %d sites for %d state model \n", i,
							assignments[i], modelStates[s]);
...
void branchLength_parallelReduce(tree *tr, double *dlnLdlz, double *d2lnLdlz2) {
	...
	memcpy(d2lnLdlz2, globalResult + tr->numBranches,
			sizeof(double) * tr->numBranches);
#endif
}
 ...
 */
static int doublesToBuffer(double *buf, double *srcTar, tree *tr, int n,
		int tid, boolean read, boolean countOnly) {
	int model, i;
	double *initPtr = buf;
...
void broadcastAfterRateOpt(tree *tr, tree *localTree, int n, int tid) {
	...
	for (i = 0; i < n; ++i) {
		double allBuf[tr->originalCrunchedLength * 3],
		...
		num1 = doublesToBuffer(buf1, localTree->patrat, tr, n, i, TRUE,
				i != tid);
		num2 = doublesToBuffer(buf2, localTree->patratStored, tr, n, i, TRUE,
				i != tid);
		...
		assertCtr += doublesToBuffer(buf2, tr->patratStored, tr, n, i, FALSE,
				FALSE);
...
static void collectDouble(double *dst, double *src, tree *tr, int n, int tid) {
	int i;
	double resultBuf[tr->originalCrunchedLength],
			buf[tr->originalCrunchedLength];
	int assertNum, displacements[tr->numberOfThreads];
...
static void broadCastAlpha(tree *localTree, tree *tr, int tid) {
	...
	int i, bufSize = localTree->NumberOfModels * 4 * sizeof(double);

	char bufDbl[bufSize], *bufPtrDbl = bufDbl;

	RECV_BUF(bufDbl, bufSize, MPI_BYTE);

	for (model = 0; model < localTree->NumberOfModels; model++)
		for (i = 0; i < 4; ++i)
			ASSIGN_BUF_DBL(localTree->partitionData[model].gammaRates[i],
					tr->partitionData[model].gammaRates[i]);
...
static void broadCastRates(tree *localTree, tree *tr, int tid) {
	...
	for (model = 0; model < localTree->NumberOfModels; model++) {
		const partitionLengths *pl = getPartitionLengths(
				&(tr->partitionData[model])); /* this is constant, isnt it?  */

		for (i = 0; i < pl->eignLength; ++i)
			ASSIGN_BUF_DBL(localTree->partitionData[model].EIGN[i],
					tr->partitionData[model].EIGN[i]);
		for (i = 0; i < pl->evLength; ++i)
			ASSIGN_BUF_DBL(localTree->partitionData[model].EV[i],
					tr->partitionData[model].EV[i]);
		for (i = 0; i < pl->eiLength; ++i)
			ASSIGN_BUF_DBL(localTree->partitionData[model].EI[i],
					tr->partitionData[model].EI[i]);
		for (i = 0; i < pl->tipVectorLength; ++i)
			ASSIGN_BUF_DBL(localTree->partitionData[model].tipVector[i],
					tr->partitionData[model].tipVector[i]);
	}
	SEND_BUF(bufDbl, bufSize, MPI_BYTE); /*  */
}
...
static void reduceEvaluateIterative(tree *localTree, int tid) {
	...
	MPI_Reduce(localTree->perPartitionLH, buf, localTree->NumberOfModels,
...
ASSIGN_BUF		(localTree->td[0].executeModel[i], tr->td[0].executeModel[i], int);
	...
	if (treeIsInitialized && localTree->td[0].count > TRAVERSAL_LENGTH) {
		...
		MPI_Bcast(localTree->td[0].ti + TRAVERSAL_LENGTH,
				localTree->td[0].count - TRAVERSAL_LENGTH, TRAVERSAL_MPI, 0,
				MPI_COMM_WORLD);
	}
#endif
}
...
double sum;
	...
	case THREAD_EVALUATE:
		...
		sum = 0;
		for (i=0; i<16; i++)
			sum += localTree->partitionData[0].tipVector[i];
		break;
	case THREAD_MAKENEWZ_FIRST:
	...
	case THREAD_MAKENEWZ: {
		...
		MPI_Reduce(dlnLdlz, result, localTree->numBranches, MPI_DOUBLE, MPI_SUM,
				0, MPI_COMM_WORLD);
		if (MASTER_P)
			memcpy(globalResult, result,
					sizeof(double) * localTree->numBranches);

		memset(result, 0, localTree->numBranches * sizeof(double));
		MPI_Reduce(d2lnLdlz2, result, localTree->numBranches, MPI_DOUBLE,
				MPI_SUM, 0, MPI_COMM_WORLD);
		if (MASTER_P)
			memcpy(globalResult + localTree->numBranches, result,
					sizeof(double) * localTree->numBranches);
#endif
	}
	...
	case THREAD_COPY_INIT_MODEL: {
		...
		if (localTree->rateHetModel == CAT) /* TRICKY originally this should only be executed by workers  */
		{
			...
			for (model = 0; model < localTree->NumberOfModels; model++)
				localTree->partitionData[model].numberOfCategories =
						tr->partitionData[model].numberOfCategories;
	...
	case THREAD_RATE_CATS: {
		...
		optRateCatPthreads(localTree, localTree->lower_spacing,
				localTree->upper_spacing, localTree->lhs, n, tid);
	...
	case THREAD_COPY_RATE_CATS: {
		...
		char bufDbl[dblBufSize], *bufPtrDbl = bufDbl;
		...
		for (model = 0; model < localTree->NumberOfModels; ++model)
			for (i = 0; i < localTree->partitionData[model].numberOfCategories;
					i++)
				ASSIGN_BUF_DBL(localTree->partitionData[model].perSiteRates[i],
						tr->partitionData[model].perSiteRates[i]);
	...
	MPI_Bcast(tr->rateCategory, tr->originalCrunchedLength, MPI_INT, 0, MPI_COMM_WORLD); 
		...
		for (model = 0; model < localTree->NumberOfModels; model++) {
			if (localTree->manyPartitions) {
				if (isThisMyPartition(localTree, tid, model))
					for (localCounter = 0, i =
							localTree->partitionData[model].lower;
							i < localTree->partitionData[model].upper;
							i++, localCounter++) {
						localTree->partitionData[model].rateCategory[localCounter] =
								tr->rateCategory[i];
					}
			} else {
				for (localCounter = 0, i = localTree->partitionData[model].lower;
						i < localTree->partitionData[model].upper; i++) {
					if (i % n == tid) {
						localTree->partitionData[model].rateCategory[localCounter] =
								tr->rateCategory[i];
...
void *likelihoodThread(void *tData) {
	...
	tree **tr = td->tr;
	int numberOfTrees = td->numberOfTrees;
#ifdef _USE_PTHREADS
	tree *localTree[numberOfTrees];
	int i;
	for (i=0;i<numberOfTrees;i++)
		localTree[i] = calloc(1,sizeof(tree ));

	int
	myCycle = 0;
	const int
	n = td->tr[0]->numberOfThreads,
	tid = td->threadNumber;
#ifndef _PORTABLE_PTHREADS
	pinToCore(tid);
#endif
	...
	while(1)
	{
		...
		if (activeTree<0) {
			printf("ERROR: No active tree has been found for thread %d\n", tid);
			exit(-1);
		}
		execFunction(tr[activeTree], localTree[activeTree], tid, n);
		barrierBuffer[tid] = 1;
	}
#else 

	const int n = processes, tid = ((threadData*) tData)->threadNumber;

	printf("\nThis is RAxML Worker Process Number: %d\n", tid);

	while (execFunction(tr[0], tr[0], tid, n))
		;
#endif
...
void distributeYVectors(tree *localTree, tree *tr) {
	size_t i, n = localTree->numberOfThreads, globalCounter = 0, localCounter =
			0, model = 0, j;
	...
	for (j = 1; j <= (size_t) localTree->mxtips; j++) {
		...
		for (model = 0, globalCounter = 0;
				model < (size_t) localTree->NumberOfModels; model++) {
			if (tr->manyPartitions) {
				if (isThisMyPartition(localTree, tid, model)) {
					assert(
							localTree->partitionData[model].upper - localTree->partitionData[model].lower == localTree->partitionData[model].width);
					for (localCounter = 0, i =
							(size_t) localTree->partitionData[model].lower;
							i < (size_t) localTree->partitionData[model].upper;
							i++, localCounter++, globalCounter++)
#ifdef _USE_PTHREADS
						localTree->partitionData[model].yVector[j][localCounter] = tr->yVector[j][globalCounter];
#else
						localTree->partitionData[model].yVector[j][localCounter] =
								yBuf[globalCounter];
#endif

				} else
					globalCounter += (localTree->partitionData[model].upper
							- localTree->partitionData[model].lower);
			} else {
				for (localCounter = 0, i =
						(size_t) localTree->partitionData[model].lower;
...
void distributeWeights(tree *localTree, tree *tr) {
	...
	size_t globalCounter = 0, i, localCounter = 0, model;
		...
		if (tr->manyPartitions) {
			if (isThisMyPartition(localTree, tid, model)) {
				...
				for (localCounter = 0, i =
						...
						i < (size_t) localTree->partitionData[model].upper;
						i++, localCounter++, globalCounter++)
					localTree->partitionData[model].wgt[localCounter] =
							tr->aliaswgt[globalCounter];
			} else
				globalCounter += (localTree->partitionData[model].upper
						- localTree->partitionData[model].lower);
		} else {
...
void initializePartitionsMaster(tree *tr, tree *localTree, int tid, int n) {
...
#else 
	globalResult = calloc(
			(size_t) tr->numberOfThreads * (size_t) tr->NumberOfModels * 2,
			sizeof(double));
	assignAndInitPart1(localTree, tr, &tid);
	...
	{
		size_t model, i, countOffset, myLength = 0;
		...
		localTree->y_ptr = (unsigned char *) malloc(
				myLength * (size_t) (localTree->mxtips)
						* sizeof(unsigned char));
		assert(localTree->y_ptr != NULL);

		for (i = 0; i < (size_t) localTree->mxtips; i++) {
			for (model = 0, countOffset = 0;
					model < (size_t) localTree->NumberOfModels; model++) {
				localTree->partitionData[model].yVector[i + 1] =
						&localTree->y_ptr[i * myLength + countOffset];
